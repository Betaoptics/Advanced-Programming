...
bool IApplication::Create(int32_t iResX, int32_t iResY, const std::string& strTitle)
{
	m_Window = MakeWindow(iResX, iResY, strTitle);
	if (m_Window)
	{
		m_iWidth = iResX;
		m_iHeight = iResY;
...
bool IApplication::OnEvent(UINT uMessage, WPARAM wParam, LPARAM lParam)
{
	switch (uMessage)
	{
	case WM_SIZE:
	...
	case WM_MOUSEMOVE:
		...
		if (pointerIndex > -1)
		{
			...
			{
				// ask for WM_MOUSELEAVE events
				TRACKMOUSEEVENT e;
				memset(&e, 0, sizeof(TRACKMOUSEEVENT));
...
HWND IApplication::MakeWindow(int32_t iWidth, int32_t iHeight, const std::string& strTitle)
{
	...
	// create the window
	std::wstring widestr = std::wstring(strTitle.begin(), strTitle.end());
...
long WINAPI IApplication::WndProc(HWND hwnd, UINT uMessage, WPARAM wParam, LPARAM lParam)
{
	switch (uMessage)
	{
	case WM_DESTROY:
	...
	if (pApp)
	{
		...
		callDefWndProc = !pApp->OnEvent(uMessage, wParam, lParam);
	}
